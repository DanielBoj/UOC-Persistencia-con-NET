...
namespace GenteFitOdoo.Controllers
{
    ...
    public class Api : ControllerBase
    {
        ...
        /* MANEJO ODOO */
        // A través de nuestra API de Python que manejamos desde OdooController obtenemso los objetos de Odoo.
        // Obtenemos los clientes de Odoo.
        [HttpGet("odoo/clientes")]
        public async Task<IActionResult> ListAllClientesOdoo()
        {
            // Creamos los controladores necesarios, hay que crear uno de mongo para comparar si ya existe el cliente.
            ...
            {
                // Obtenemos los clientes de Odoo.
                List<Cliente> clientesOdoo = await python.GetAllClientes();
                ...
                clientesMongo = await dbCliente.GetAllClientes();

                // Setteamos los id de Mongo en los clientes obtenidos de Odoo.
                clientesOdoo.ForEach((cliente) =>
                {
                    cliente.Id = clientesMongo.Where(clienteM => clienteM.Email.Equals(cliente.Email)).FirstOrDefault().Id;
                });

                // Devolvemos el resultado.
                return Ok(clientesOdoo);
            }
            catch (Exception err)
            {
                return StatusCode(400, err.Message);
        ...
        // Obtenemos los empleados de Odoo.
        [HttpGet("odoo/empleados")]
        public async Task<IActionResult> ListAllEmpleadosOdoo()
        {
            // Creamos el controlador de la api de python
            PythonApiController python = new();
            ...
            try
            {
                // Obtenemos los empleados de Odoo y de Mongo
                List<EmpleadoOdoo> empleados = await python.GetAllEmpleados();
                List<Empleado> empleadosMongo = await empleadoController.GetAllEmpleados();

                // Iteramos la lista de empleados, convertimos los objetos a Empleado de MongoDB, comprobamos si 
                // existen en la base de datos y los añadimos.
                empleados.ForEach(async e =>
                {
                    if (e.Email != null)
                    {
                        // Si el empleado es el administrador por defecto de Odoo, lo ignoramos.
                        if (e.Name.Contains("Administrator")) return;

                        Empleado empleado = new(e.Email, "claptrap")
                        {
                            Tipo = "empleado"
                        };

                        // Recorremos la lista de empleados de MongoDB para comprobar si existe.
                        if (!empleadosMongo.Any(empleadoMongo => empleadoMongo.Email.Equals(empleado.Email)))
                        {
                            // Si no existe, lo añadimos a la base de datos.
                            await empleadoController.Create(empleado);
                        }
                    }
                });

                // Eliminamos el administrador por defecto de Odoo.
                empleados.RemoveAll(e => e.Name.Contains("Administrator"));
        ...
        // Obtenemos los pedidos de Odoo
        [HttpGet("odoo/pedidos")]
        public async Task<IActionResult> ListAllProductos()
        {
            // Creamos el controlador de la api de python
            PythonApiController python = new();
            // Obtenemos los pedidos de Odoo.
            try
            {
                List<Pedido> pedidos = await python.GetAllPedidos();
                // Devolvemos el resultado.
                return Ok(pedidos);
            }
            catch (Exception err)
